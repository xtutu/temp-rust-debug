        ...
        let ret = lua_ctx
            .load(
                ...
                if protobuf ~= nil then
                    print("load protobuf success")
                else
                    print("load protobuf fail")
                end
                local x = { a = 100, b = "hello json"}
                print("json:", Json.encode(x))
            "#,
            )
            .exec();
        if ret.is_err(){
            log::error!("{:?}", ret.unwrap_err().to_string());
        }
        Ok(())
    })
}