...
//@lyx
impl Lua{
    pub unsafe fn get_context(&self)->Context<'static>{
        return Context::<'static>::new(self.main_state)
    }
}
...
unsafe fn load_from_std_lib(state: *mut ffi::lua_State, lua_mod: StdLib) {
    ...
    ffi::luaL_requiref(state, cstr!("pb"), ffi::luaopen_pb, 1);
    ffi::lua_pop(state, 1);

    ffi::luaL_requiref(state, cstr!("rapidjson"), ffi::luaopen_rapidjson, 1);
    ffi::lua_pop(state, 1);

    // ffi::luaL_requiref(state, cstr!("socket"), ffi::luaopen_socket_core, 1);
    // ffi::lua_pop(state, 1);
